...
resource_files:
  - a4learn-pacc-lcmm-base.rdata
  - a4learn-pacc-lcmm-tau.rdata
  - cross-val-base-model-fits.rdata
  - cross-val-tau-pet-model-fits.rdata
  - tree_cv_base_results.rdata
  - tree_cv_tau_pet_results.rdata
author:
  - name: Runpeng Li
...
qs_pacc <- A4LEARN::ADQS %>% filter(QSTESTCD %in% "PACC") %>%
  ...
  select(BID, SUBSTUDY, VISITCD, ASEQNCS, AVISIT, ADURW, AMYLCENT, Y, TX, 
    ...
    Entorhinal, Hippocampus, Hipp_atrophy, `CDR Progressor`) %>%
  ungroup() %>%
...
Clinical relevance of the PACC latent classes was supported by differences in functional outcomes. The proportion of participants who were observed to progress on the Clinical Dementia Rating (CDR) Global score increased monotonically across latent classes, with the lowest rate among non-decliners (22.8%) and progressively higher rates among slow- (71.9%) and fast-decliners (88.2%) (@tbl-baseline-base-characteristics). However, discordance between PACC latent class labels and observed CDR progression is common.
