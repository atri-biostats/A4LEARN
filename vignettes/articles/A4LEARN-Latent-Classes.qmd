...
affiliations:
  ...
  - id: MGH
    ...
    state: MA
abstract: |
  **Importance:** It has been shown that Alzheimer’s disease biomarkers in cognitively unimpaired older adults are associated with subsequent cognitive and clinical decline. However, considerable heterogeneity in the rate and pattern of decline remains unexplained.

  **Objective:** To characterize subgroups of cognitive decline among biomarker-defined cognitively unimpaired adults and identify predictors of heterogeneity in preclinical Alzheimer’s disease progression.
  
  **Design, Setting, and Participants:** We analyzed longitudinal data from participants in the A4 Study (amyloid-positive) and the LEARN Study (amyloid-negative) who completed baseline amyloid PET, plasma p-tau217, and serial cognitive assessments. Latent Class Mixed-Effects Models (LCMMs) were used to identify distinct patterns of cognitive change over time. Associations between class membership and baseline demographic, clinical, and biomarker variables were evaluated.
  
  **Main Outcomes and Measures:** The primary outcome was change in the Preclinical Alzheimer’s Cognitive Composite (PACC) score over time. Secondary outcomes included class-specific rates of decline and predictors of class membership.
  
  **Results:** Three distinct classes of cognitive trajectories were identified, representing stable, slow decline, and fast decline. Higher baseline plasma P‐tau217 levels, greater baseline hippocampal atrophy, and higher baseline tau PET levels were associated with membership in declining subgroups. Among amyloid-positive participants, approximately 70% were classified as non-decliners over the course of observation.
  
  **Conclusions and Relevance:** Latent class modeling reveals marked heterogeneity in cognitive decline among individuals with and without biomarker evidence of Alzheimer’s pathology. The high proportion of non-decliners is not unexpected given that the presymptomatic phase of Alzheimer disease may span more than a decade. Identifying subgroups of decline may enhance the design of precision prevention trials and improve prognostic modeling in preclinical Alzheimer’s disease.
keywords: 
...
qs_pacc <- A4LEARN::ADQS %>% filter(QSTESTCD %in% "PACC") %>%
  left_join(A4LEARN::ptdemog %>% select(BID, Sex = PTGENDER, PTRACE, PTETHNIC),
    by='BID') %>%
  ...
  select(BID, SUBSTUDY, VISITCD, ASEQNCS, AVISIT, ADURW, AMYLCENT, Y, TX, 
    ...
    Entorhinal, Hippocampus, Hipp_atrophy) %>%
  ungroup() %>%
...
Analyses were conducted using harmonized data from the A4 and LEARN studies, accessed through the `A4LEARN` R data package (version 1.1.20250808 @donohue2025alzheimer), available at <https://github.com/atri-biostats/A4LEARN>. This package provides curated datasets and metadata derived from the A4 and LEARN clinical studies for reproducible statistical analysis. Code to reproduce the results from this paper are available from <https://github.com/atri-biostats/A4LEARN/tree/main/vignettes/articles>.
...
To assess the reliability and potential predictive utility of the latent class membership model, we examined the posterior probabilities of class assignment for each participant. High posterior probabilities indicate greater certainty in latent class classification. We summarized these distributions across classes and computed the mean posterior probability within each group as an index of classification confidence. To further evaluate prospective discrimination, we performed ten-fold cross-validation. For each of the ten folds, 90% of the data were used to re-train the LCMM, and the re-trained model was used to predict latent classes for the 10% held-out test set using only baseline data. Model performance was summarized using the area under the precision-recall curve (AUPRC) for discrimination of each class versus all others. This approach is preferred over area under the receiver operating characteristic (ROC) curve when group sizes are imbalance.

## Regression Tree Analysis for Latent Class Characterization

To evaluate whether latent classes could be distinguished by baseline characteristics through a combination of binary decision rules, we conducted a post-hoc classification tree analysis [@rpart; @breiman2017classification]. Class assignments from the optimal latent class mixed model were used as the outcome variable, with baseline demographic and clinical characteristics as predictors (randomized treatment group, plasma P-tau217 concentration, florbetapir PET, APOE $\epsilon 4$ carrier status, sex, age, education, hippocampal atrophy, and PACC). We separately considered a regression tree with tau PET as an additional predictor.
...
# make non-decliner last so that it is the reference group
...
qs_pacc <- qs_pacc %>%
  ...
  mutate(`Class (Base)` =
    case_when(
      ...
      class == 3 ~ 'Non-decliner',) %>% 
      factor(levels = c('Non-decliner', 'Slow decliner', 'Fast decliner'))) %>%
  select(-class) %>%
...
#| results: asis
#| tbl-cap: "**Baseline Demographic, Clinical, and Biomarker Characteristics by Latent Class of Cognitive Decline.** Baseline characteristics of participants classified by the latent class mixed model (LCMM) of Preclinical Alzheimer’s Cognitive Composite (PACC) trajectories. Classes represent distinct longitudinal cognitive patterns: non-decliners, slow decliners, and fast decliners. Continuous variables are presented as mean (SD); categorical variables as No. (%). **Abbreviations:** APOE = apolipoprotein E; PET = positron emission tomography; P-tau217 = plasma phosphorylated tau 217; SUVr = standardized uptake value ratio; U/mL = arbitrary units proportional to assay signal; Am. = American; PI = Pacific Islander. **Footnotes:** Amyloid PET and tau PET SUVr values represent mean cortical uptake relative to cerebellar reference region. Hippocampal atrophy are residualized for intracranial volume and z-scored. Education reported in years of formal schooling."

mylabels <- list(Y = "PACC, mean (SD)", AMYLCENT = "Amyloid PET, mean (SD), CL", 
  EDCCNTU = 'Education, mean (SD), y', TX = 'Group, No. (%)', AGEYR = 'Age, mean (SD), y',
  Ptau217 = "P-tau217, mean (SD), U/ml", Tau_PET = "Tau PET, mean (SD), SUVr",
  Hipp_atrophy_z = "Hipp. atrophy, mean (SD), z-score", Sex = "Sex, No. (%)", Race = "Race, No. (%)", Ethnicity = "Ethnicity, No. (%)", APOEe4 = "APOEε4, No. (%)")
...
## Identification and Prediction of Latent Classes
...
data_pred <- bl_summaries %>%
  ...
  cross_join(tibble(ADURW = seq(0, 456, by = 24)))
...
pred <- predictY(m.best.mri, data_pred, var.time = "ADURW", draws = TRUE)$pred %>%
  ...
  mutate(
    class = case_when(
      ...
      grepl('class3', name) ~ 'Non-decliner') %>% 
      factor(levels = c('Non-decliner', 'Slow decliner', 'Fast decliner')),
    estimate = case_when(
...
p1 <- ggplot(pred, aes(x=Week, y=prediction_raw, group = class)) +
  ...
  guides(color = guide_legend(title = "PACC class", override.aes = list(size = 5)), fill = "none") +
  theme_jama() +
  theme(legend.position = 'inside', legend.position.inside = c(0.3, 0.2)) +
  scale_color_jama() +
  scale_fill_jama()

p2 <- qs_pacc %>% filter(!is.na(`Class (Base)`)) %>%
...
@fig-lcmm-coefs Panels B and D presents standardized coefficients from the longitudinal submodel, representing associations between each covariate and the PACC outcome. Higher PACC scores correspond to better cognitive performance and continuous variables are standardized and oriented so that positive coefficients indicate better performance. We see that females had increased risk of being fast decliners (OR 2.5, 95% CI 1.5 to 4.4), but had better PACC performance for a given class membership (0.30 PACC points, 95% CI 0.24 to 0.35). APOE $\epsilon 4$ carriage was associated with increased risk of both slow (OR 1.9, 95% CI 1.3 to 2.8) and fast (OR 2.9, 95% CI 1.6 to 5.3) declining class versus non-declining class, but negligible remaining effect on PACC. Education had no effect on class membership, but was associated with better PACC performance (0.12 PACC points per SD of education, 95% CI 0.095 to 0.15).

## Predictive Performance and Cross-Validation

The overall cross-validated accuracy was 0.80 (95% CI 0.79 to 0.82) for the base model and 0.756 (95% CI 0.73 to 0.78) for the tau PET model. However, this accuracy rate is inflated by the large number of non-decliners (1,257/1,629 = 77.2%), which are relatively easy for the model to identify. AUPRCs from the base model were 0.95, 0.41, and 0.49 for non-, slow-, and fast decliners versus the rest; and 0.70 for either slow- or fast decliner vs non-decliner (@suppfig-cross-val-base-pr). AUPRCs from the tau PET model were 0.92, 0.31, and 0.56 for non-, slow-, and fast decliners versus the rest; and 0.71 for either slow- or fast decliner vs non-decliner (@suppfig-cross-val-tau-pet-pr).

## Post-hoc Regression Tree

The optimally-tuned classification tree without tau PET achieved a mean balanced accuracy of 0.63 (standard error 0.013) in 10-fold cross-validation (@fig-tree-base). Variable importance analysis identified baseline P-tau217 (56.1%), baseline PACC (15.5%), amyloid PET (13.1%), and hippocampal atrophy (10.0%) as most important; though amyloid PET was not included in the tree, likely due to its correlation with P-tau217. The tree structure revealed a hierarchical decision process with participants with baseline P-tau217 \< 0.29 were classified as non-decliners, though this miss-classifies $N=155$ decliners. Those with P-tau217 $\geq$ 0.29 are further parsed by hippocampal atrophy, PACC, and P-tau217. Only one leaf contains a majority of fast decliners, and it contains only 1% of the initial sample. While the tree demonstrates the relative importance and utility of the predictors, it also demonstrates a high degree of miss-classification (18.6%). Cross-validated balanced accuracy is improved by the addition of tau PET (mean 0.70, standard error 0.011; @suppfig-tree-tau-pet). Tau PET has the second largest variable importance (22.5%), between P-tau217 (29.8%) and hippocampal atrophy (17.4%).
...
Although plasma P-tau217 and amyloid PET were each associated with latent class membership, neither biomarker alone, or in combination, achieved adequately accurate prediction of cognitive decline. A key observation was the high frequency of cognitive stability among amyloid-positive participants. As shown in @tbl-baseline-base-characteristics, 776 of 1,110 amyloid-positive A4 participants (69.9%) were classified as non-decliners, indicating that substantial amyloid burden does not necessarily translate into measurable cognitive change over the timescales typical of prevention trials. However, this is consistent with a pre-symptomatic phase of disease believed to last as long as 15 years.
...
**Participant Selection.** Because the majority of amyloid-positive individuals remained cognitively stable, restricting eligibility based solely on amyloid positivity will invariably enroll many non-decliners, reducing statistical power for cognitive endpoints. Plasma P-tau217 may aid enrichment but is insufficient to isolate a predominantly decliner cohort. More complex multimodal strategies or repeated biomarker assessments may be required to achieve adequate prognostic discrimination.

**Modeling Treatment Effects in Heterogeneous Cohorts.** Latent class distinctions complicate assumptions about expected treatment benefit. Fast decliners may show larger measurable benefit yet may represent individuals further along the disease continuum. Non-decliners present an additional challenge: their stability limits the detectable cognitive benefit even if biological effects occur. Power simulations and trial analyses may therefore need to incorporate class-specific trajectories or assume differential treatment effects across latent classes.
...
pred <- predictY(m.best.tau, data_pred, var.time = "ADURW", draws = TRUE)$pred %>%
  ...
  mutate(
    class = case_when(
      ...
      grepl('class3', name) ~ 'Non-decliner') %>% 
      factor(levels = c('Non-decliner', 'Slower decliner', 'Faster decliner')),
    estimate = case_when(
      grepl('lower', name) ~ 'lower',
      grepl('upper', name) ~ 'upper',
      TRUE ~ 'prediction'),
...
p2 <- qs_pacc %>% filter(!is.na(`Class (Tau PET)`)) %>%
  ggplot(aes(x = ADURW, y=Y, group = id)) +
...
**Latent class mixed model identifies distinct cognitive decliner trajectories.** (a) Predicted PACC scores for participants classified as cognitive decliners (red) or non-decliners (blue) over time using a latent class mixed model. Shaded regions represent 95% confidence intervals. Vertical dashed line at week 240 marks the start of Open Label Extension. (b) Individual participant trajectories colored by decliner classification, showing heterogeneity within each group.
...
# Either fast- or slow ----
bin_class_data <- pr_df %>% 
  filter(class_label == "Non-decliner") %>%
  mutate(
    pred_bin_class = case_when(
      predicted_class == "Non-decliner" ~ 0,
      TRUE ~ 1),
    true_bin_class = case_when(
      true_class == "Non-decliner" ~ 0,
      TRUE ~ 1))
pr_obj <- pr.curve(
...
for(fold in 1:n_folds) {
  ...
  labs <- tibble(
    class_raw = colnames(labs_data)[c(non, slow, fast)],
    class_number = gsub('Ypred_class', '', class_raw) %>% as.numeric(),
    predicted_class = factor(c('Non-decliner', 'Slow decliner', 'Fast decliner'),
      levels = levels(qs_pacc$`Class (Base)`)))
...
roc_plot_data <- do.call(rbind, roc_list) %>%
  ...
  mutate(Class = factor(paste0(class, " (AUC = ", 
    format(round(auc, 3), nsmall = 3), ")"),
    levels = c("Non-decliner (AUC = 0.841)",
...
tree_data <- qs_pacc %>% 
  filter(VISITCD == '006' & !is.na(Ptau217) & !is.na(APOEe4)) %>%
  select(Class = `Class (Base)`, Group, Age = AGEYR, Sex, Edu. = EDCCNTU,
    APOEe4, PACC = Y, `Amyloid PET` = AMYLCENT, Ptau217, 
...
tree_recipe <- recipe(
  Class ~ Group + Age + Sex + Edu. + APOEe4 + 
...
tree_model <- rpart(Class ~ Group + Age + Sex + Edu. + APOEe4 +
    PACC + `Amyloid PET` + Ptau217 + `Hipp atrophy`,
  data = tree_data,
  method = "class",
...
tree_data <- qs_pacc %>% 
  filter(VISITCD == '006' & !is.na(Ptau217) & !is.na(APOEe4)) %>%
  select(Class = `Class (Tau PET)`, Group, Age = AGEYR, Sex, Edu. = EDCCNTU,
    APOEe4, PACC = Y, `Amyloid PET` = AMYLCENT, Ptau217, 
    `Hipp atrophy` = Hipp_atrophy_z, `Tau PET` = Tau_PET) %>%
  na.omit()
