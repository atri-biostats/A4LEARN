...
affiliations:
  ...
  - id: MGH
    ...
    state: MA
abstract: |
  **Introduction:** Biomarkers identify Alzheimer disease pathology in cognitively unimpaired adults, but the timing and rate of cognitive decline vary widely. This study aimed to identify subgroups of cognitive decline and baseline predictors of heterogeneity in preclinical progression.
  
  **Methods:** Data were drawn from the A4 Study, which enrolled amyloid positive participants, and the LEARN Study, which enrolled amyloid negative individuals. Latent class mixed effects models identified cognitive trajectory classes. Associations between class membership and demographic, clinical, and biomarker variables were evaluated. The primary outcome was change in the Preclinical Alzheimer Cognitive Composite.

  **Results:** Three trajectory classes were identified: stable, slow decliners, and fast decliners. Higher P-tau217, smaller hippocampal volume, and elevated tau PET were associated with declining classes. About 70 percent of amyloid positive individuals were stable.
...
scale_vec <- function(x, ...){
  y <- scale(x, ...)
  sc <- attr(y, "scaled:center")
  ss <- attr(y, "scaled:scale")
  y <- y[,1]
  attr(y, "scaled:center") <- sc
  attr(y, "scaled:scale") <- ss
  y
}
```
...
qs_pacc <- A4LEARN::ADQS %>% filter(QSTESTCD %in% "PACC") %>%
  ...
  mutate(Y = QSSTRESN, Y_ch = QSCHANGE, Y_bl = QSBLRES,
  ...
  left_join(mri_data %>% 
    ...
    by = 'BID') %>%
  arrange(BID, ADURW) %>%
  select(BID, SUBSTUDY, EPOCH, VISITCD, ASEQNCS, ASEQMMRM, AVISIT, ADURW, AMYLCENT, 
    Y, Y_ch, Y_bl, TX, Group, AGEYR, Sex, Female, Race, Ethnicity, EDCCNTU, 
    QSVERSION, AAPOEGNPRSNFLG, APOEe4, SUVRCER, Ptau217, Sola, Tau_PET, 
    IntraCranialVolume, Entorhinal, Hippocampus, Hipp_atrophy, 
    MEMORY, CDMEM, `CDR Progressor`) %>%
  ungroup() %>%
...
qs_pacc <- qs_pacc %>%
  mutate(
    Z = scale_vec(Y_boxcox),
    AGEYR_z = scale_vec(AGEYR),
    EDCCNTU_z = scale_vec(EDCCNTU),
    Hipp_atrophy_z = scale_vec(Hipp_atrophy),
    Ptau217_z = scale_vec(Ptau217),
    SUVRCER_z = scale_vec(SUVRCER),
    Tau_PET_z = scale_vec(Tau_PET))
...
The dependent variable was the Box-Cox-transformed Preclinical Alzheimer Cognitive Composite (PACC)[@Donohue2014] score at each visit. The Box-Cox transformation parameter ($\lambda$) was selected to maximize the likelihood that the transformed data approximated normality. Time was modeled as a continuous variable representing years since baseline.
...
```{r load-lcmm-results, include = FALSE, eval = UPDATELCMM}
...
save(m.best.mri, m.best.tau, file = "a4learn-pacc-lcmm-best-models.rdata")
```

```{r attach-class-labels}
load('a4learn-pacc-lcmm-base.rdata')
load('a4learn-pacc-lcmm-tau.rdata')
load("a4learn-pacc-lcmm-best-models.rdata")
...
data_pred_mri <- qs_pacc %>%
  select(`Class (Base)`, id, Female, Sola, AAPOEGNPRSNFLG, SUVRCER_z, Ptau217_z, 
    AGEYR_z, EDCCNTU_z, Hipp_atrophy_z) %>%
  distinct(.keep_all = TRUE) %>%
  pivot_longer(cols = Female:Hipp_atrophy_z, names_to = 'variable', values_to = 'value') %>%
  group_by(variable, `Class (Base)`) %>%
  summarise(mean = mean(value)) %>%
  pivot_wider(names_from = variable, values_from = mean) %>%
  mutate(QSVERSION = sort(unique(qs_pacc$QSVERSION))[1]) %>%
  cross_join(tibble(ADURW = seq(0, 425, by = 25)))

pred_stable <- 
  predictY(m.best.mri, data_pred_mri %>% filter(`Class (Base)` == 'Stable'), 
    var.time = "ADURW", draws = TRUE)$pred %>%
  as_tibble() %>%
  bind_cols(data_pred_mri %>% filter(`Class (Base)` == 'Stable') %>% 
      select(`Class (Base)`, Week = ADURW)) %>%
  select(`Class (Base)`, Week, prediction = Ypred_class3, 
    lower = lower.Ypred_class3, upper = upper.Ypred_class3)

pred_slow <- 
  predictY(m.best.mri, data_pred_mri %>% filter(`Class (Base)` == 'Slow decliner'), 
    var.time = "ADURW", draws = TRUE)$pred %>%
  as_tibble() %>%
  bind_cols(data_pred_mri %>% filter(`Class (Base)` == 'Slow decliner') %>% 
      select(`Class (Base)`, Week = ADURW)) %>%
  select(`Class (Base)`, Week, prediction = Ypred_class1, 
    lower = lower.Ypred_class1, upper = upper.Ypred_class1)

pred_fast <- 
  predictY(m.best.mri, data_pred_mri %>% filter(`Class (Base)` == 'Fast decliner'), 
    var.time = "ADURW", draws = TRUE)$pred %>%
  as_tibble() %>%
  bind_cols(data_pred_mri %>% filter(`Class (Base)` == 'Fast decliner') %>% 
      select(`Class (Base)`, Week = ADURW)) %>%
  select(`Class (Base)`, Week, prediction = Ypred_class2, 
    lower = lower.Ypred_class2, upper = upper.Ypred_class2)

pred <- bind_rows(pred_stable, pred_slow, pred_fast) %>%
  mutate(
    Years = Week * 7 / 365.25,
...
p1 <- qs_pacc %>% filter(!is.na(`Class (Base)`)) %>%
  mutate(Years = ADURW * 7 / 365.25) %>%
  ggplot(aes(x = Years, y=Y, group = id)) +
...
p2 <- ggplot(pred, aes(x=Years, y=prediction_raw, group = `Class (Base)`)) +
  geom_line(aes(color = `Class (Base)`)) +
  geom_ribbon(aes(ymin = lower_raw, ymax = upper_raw, fill = `Class (Base)`), alpha = 0.2) +
...
em_effect <- em_all %>%
  ...
  mutate(
    `Delta (%)` = case_when(
      Group == 'Decliners' ~  20,
      Group == 'Stable Amyloid+' ~  100),
    Delta = `Delta (%)`/100 * (upper_limit - emmean),
    Treated = case_when(
      Years == 0 ~ emmean,
      TRUE ~ emmean + Delta))

ggplot(em_all, aes(x=Years, y=emmean)) +
  geom_line(aes(color=Group)) +
  geom_line(data=em_effect, aes(x=Years, y=Treated, color=Group), 
    linetype='dashed') +
  geom_point(data=em_effect %>% filter(Years %in% c(2,4)), 
    aes(x=Years, y=Treated, color=Group), 
    shape=3) +
  ylab("Mean PACC Score") +
  theme_jama() +
  scale_color_manual(values = c("#B24745", "#79AF97", "#6A6599")) +
  theme(
    legend.position = 'inside', 
    legend.position.inside = c(0.2, 0.2))
```

Power analyses stratified by latent class revealed marked differences in cognitive trajectories and variability across classes (see @supptbl-clda-power). The amyloid-negative stable group had the highest mean (SD) PACC scores over time: +1.14 (2.11) at 2 years and +1.20 (2.25) at 4 years. We considered this the maximum possible treatment benefit (see @fig-clda-power). The stable amyloid-positive group had means of +0.88 (2.19) at 2 years and +0.96 (2.34) at 4 years. With 500 participants per arm, a trial in amyloid-positive stable individuals would have only 44% power at 2 years and 30% at 4 years to detect even 100% of the maximum benefit (0.26 PACC points at 2 years and 0.24 at 4 years). In contrast, a trial in amyloid-positive declining individuals would have 93% power at 2 years and 98% at 4 years to detect just 20% of the maximum benefit (0.79 PACC points at 2 years and 1.40 at 4 years).

These findings suggest that stable amyloid-positive individuals, although they may have the greatest potential long-term benefit from a disease-modifying drug, contribute little to the statistical power of a preclinical Alzheimer's disease trial to detect an effect on the PACC. Instead, power is primarily driven by declining individuals, who represent a minority of the amyloid-positive population. Enrolling large numbers of stable individuals may dilute the overall treatment effect and reduce the trial's ability to detect meaningful cognitive benefits. These results underscore the need to account for latent classes of decline when designing preclinical Alzheimer's disease trials. Again, the goal of this analysis was not to suggest conducting trials in homogeneous populations. Rather, we aimed to highlight the complexity and potential inefficiency introduced when multiple latent classes with distinct trajectories are present within a trial cohort.
...
```{r suppfig-lcmm-coefs, fig.width = 7, fig.height = 6}
## mri model ----
...
tbl.or.tau.raw <- tmp.or
tbl.long.tau.raw <- tmp.long
...
**Association of baseline predictors with latent class membership and longitudinal cognitive decline in models with and without tau PET.** Panels A and B display results from the latent class mixed model (LCMM) excluding tau PET; panels C and D show corresponding results from the model including tau PET as an additional predictor. (A, C) Odds ratios (95% CIs) from the LCMM class-membership submodel represent the relative likelihood of belonging to the slow- or fast-declining classes relative to stable class. (B, D) Standardized coefficients (95% CIs) from the LCMM longitudinal submodel show associations between baseline predictors and rate of change in the Preclinical Alzheimer Cognitive Composite (PACC). All continuous predictors and the PACC outcome were standardized (z-scored) before analysis to enable comparison of effect magnitudes. Higher PACC scores indicate better cognitive performance; thus, larger positive coefficients correspond to slower decline or better preservation of cognition over time.

:::

::: {#supptbl-lcmm-raw-coefs}

```{r supptbl-lcmm-raw-coefs}
#| results: asis
#| tbl-colwidths: [2,10,2,10,5,10,5]

format.ci <- function(x, digits = 2, ...){
  fx <- format(x, digits = digits, ...)
  paste0(fx[1], " (", fx[2], ", ", fx[3], ")")
}

tbl.comb.long <- bind_rows(
  tbl.long.mri.raw %>% mutate(Model = 'Base model', Part = "Longitudidnal"),
  tbl.long.tau.raw %>% mutate(Model = 'Tau PET model', Part = "Longitudidnal")) %>%
  select(Model, Part, Parameter, Estimate, lwr, upr, `P Value`) %>%
  bind_rows(
    bind_rows(
      tbl.or.mri.raw %>% mutate(Model = 'Base model', Part = "Class membership"),
      tbl.or.tau.raw %>% mutate(Model = 'Tau PET model', Part = "Class membership")) %>%
      select(Model, Part, Parameter, Estimate = OR, lwr = OR.lwr, upr = OR.upr, `P Value`)) %>%
  mutate(
    Estimate = case_when(
      grepl("P-tau217", Parameter) ~ Estimate * attr(qs_pacc$Ptau217_z, "scaled:scale"),
      grepl("Amyloid PET", Parameter) ~ Estimate * attr(qs_pacc$SUVRCER_z, "scaled:scale"),
      grepl("Age", Parameter) ~ Estimate * attr(qs_pacc$AGEYR_z, "scaled:scale"),
      grepl("Education", Parameter) ~ Estimate * attr(qs_pacc$EDCCNTU_z, "scaled:scale"),
      grepl("Hipp. atrophy", Parameter) ~ Estimate * attr(qs_pacc$Hipp_atrophy_z, "scaled:scale"),
      grepl("Tau PET", Parameter) ~ Estimate * attr(qs_pacc$Tau_PET_z, "scaled:scale"),
      TRUE ~ Estimate),
    lwr = case_when(
      grepl("P-tau217", Parameter) ~ lwr * attr(qs_pacc$Ptau217_z, "scaled:scale"),
      grepl("Amyloid PET", Parameter) ~ lwr * attr(qs_pacc$SUVRCER_z, "scaled:scale"),
      grepl("Age", Parameter) ~ lwr * attr(qs_pacc$AGEYR_z, "scaled:scale"),
      grepl("Education", Parameter) ~ lwr * attr(qs_pacc$EDCCNTU_z, "scaled:scale"),
      grepl("Hipp. atrophy", Parameter) ~ lwr * attr(qs_pacc$Hipp_atrophy_z, "scaled:scale"),
      grepl("Tau PET", Parameter) ~ lwr * attr(qs_pacc$Tau_PET_z, "scaled:scale"),
      TRUE ~ lwr),
    upr = case_when(
      grepl("P-tau217", Parameter) ~ upr * attr(qs_pacc$Ptau217_z, "scaled:scale"),
      grepl("Amyloid PET", Parameter) ~ upr * attr(qs_pacc$SUVRCER_z, "scaled:scale"),
      grepl("Age", Parameter) ~ upr * attr(qs_pacc$AGEYR_z, "scaled:scale"),
      grepl("Education", Parameter) ~ upr * attr(qs_pacc$EDCCNTU_z, "scaled:scale"),
      grepl("Hipp. atrophy", Parameter) ~ upr * attr(qs_pacc$Hipp_atrophy_z, "scaled:scale"),
      grepl("Tau PET", Parameter) ~ upr * attr(qs_pacc$Tau_PET_z, "scaled:scale"),
      TRUE ~ upr),
    Parameter = as.character(Parameter),
    Parameter = gsub("P-tau217", "P-tau217 (U/ml)", Parameter),
    Parameter = gsub("Amyloid PET", "Amyloid PET (SUVr)", Parameter),
    Parameter = gsub("Age", "Age (yrs)", Parameter),
    Parameter = gsub("Education", "Education (yrs)", Parameter),
    Parameter = gsub("Hipp. atrophy", "Hipp. atrophy (cc$^3$)", Parameter),
    Parameter = gsub("Tau PET", "Tau PET (SUVr)", Parameter),
    `P Value` = Hmisc::format.pval(`P Value`, digits = 3, eps = 0.001, nsmall=3)) %>%
  rowwise() %>%
  mutate(`Estimate (95% CI)` = format.ci(c(Estimate, lwr, upr))) %>%
  select(Model, Part, Parameter, `Estimate (95% CI)`, `P Value`)

tbl.comb <- left_join(
  tbl.comb.long %>% filter(Model == 'Base model', Part == "Class membership") %>%
    mutate(Parameter0 = gsub(' (slow)', '', Parameter, fixed = TRUE)),
  tbl.comb.long %>% filter(Model == 'Base model', Part == "Longitudidnal") %>%
    rename(Parameter0 = Parameter),
  by = "Parameter0") %>% 
  bind_rows(
    left_join(
      tbl.comb.long %>% filter(Model == 'Tau PET model', Part == "Class membership") %>%
        mutate(Parameter0 = gsub(' (slow)', '', Parameter, fixed = TRUE)),
      tbl.comb.long %>% filter(Model == 'Tau PET model', Part == "Longitudidnal") %>%
        rename(Parameter0 = Parameter),
      by = "Parameter0")) %>%
  mutate(Class = case_when(
    grepl("(slow)", Parameter) ~ "slow",
    grepl("(fast)", Parameter) ~ "fast"),
    Parameter = gsub(" (slow)", '', Parameter, fixed = TRUE),
    Parameter = gsub(" (fast)", '', Parameter, fixed = TRUE)) %>%
  select(Model = Model.x, Parameter, Class, `Estimate (95% CI).x`, `P Value.x`, 
    `Estimate (95% CI).y`, `P Value.y`)

tbl.comb %>%
  kable(col.names = c('Model', 'Parameter', 'Class', 
  'Estimate (95% CI)', 'P Value', 'Estimate (95% CI)', 'P Value'), 
    escape = FALSE, font_size = 5) %>%
  collapse_rows(c(1,2)) %>%
  add_header_above(
    c(" " = 2, "Class membership model" = 3, 
      "Longitudinal model" = 2)) %>%
  kable_styling()
```

**Association of baseline predictors (on the _raw_ scale) with latent class membership and longitudinal cognitive decline in models with and without tau PET.** The top rows display results from the latent class mixed model (LCMM) excluding tau PET, while the bottom rows show corresponding results from the model including tau PET as an additional predictor. The left side shows odds ratios (95% CIs) from the LCMM class-membership submodel represent the relative likelihood of belonging to the slow- or fast-declining classes relative to stable class. The right side shows coefficients (95% CIs) from the LCMM longitudinal submodel show associations between baseline predictors and the Preclinical Alzheimer Cognitive Composite (PACC). All continuous predictors are on the raw scale. Higher PACC scores indicate better cognitive performance; thus, larger positive coefficients correspond to slower decline or better preservation of cognition over time.
...
```{r suppfig-tree-base} 
knitr::include_graphics("suppfig-tree-base.png")
```

{{< include _suppfig-tree-base-cap.txt >}}
...
**Baseline demographic, clinical, and biomarker characteristics by latent class of cognitive decline with tau PET.** Baseline characteristics of participants classified by the latent class mixed model (LCMM) of Preclinical Alzheimer Cognitive Composite (PACC) trajectories. Classes represent distinct longitudinal cognitive patterns: stable, slow decliner, and fast decliner. Continuous variables are presented as mean (SD); categorical variables as No. (%). **Abbreviations:** APOE = apolipoprotein E; PET = positron emission tomography; P-tau217 = plasma phosphorylated tau 217; SUVr = standardized uptake value ratio; U/mL = arbitrary units proportional to assay signal; Am. = American; PI = Pacific Islander; CDR = Clinical Dementia Rating; Prog. = Progressor. **Footnotes:** Amyloid PET and tau PET SUVr values represent mean cortical uptake relative to cerebellar reference region. Hippocampal atrophy are residualized for intracranial volume and z-scored. Education reported in years of formal schooling. CDR Progressors were observed to have a CDR Global score greater than zero at two consecutive visits, or their last visit.
...
data_pred_tau <- qs_pacc %>%
  select(`Class (Tau PET)`, id, Female, Sola, AAPOEGNPRSNFLG, SUVRCER_z, Ptau217_z, 
    AGEYR_z, EDCCNTU_z, Tau_PET_z, Hipp_atrophy_z) %>%
  distinct(.keep_all = TRUE) %>%
  pivot_longer(cols = Female:Hipp_atrophy_z, names_to = 'variable', values_to = 'value') %>%
  group_by(variable, `Class (Tau PET)`) %>%
  summarise(mean = mean(value)) %>%
  pivot_wider(names_from = variable, values_from = mean) %>%
  mutate(QSVERSION = sort(unique(qs_pacc$QSVERSION))[1]) %>%
  cross_join(tibble(ADURW = seq(0, 425, by = 25)))
...
pred_stable <- 
  predictY(m.best.tau, data_pred_tau %>% filter(`Class (Tau PET)` == 'Stable'), 
    var.time = "ADURW", draws = TRUE)$pred %>%
  as_tibble() %>%
  bind_cols(data_pred_tau %>% filter(`Class (Tau PET)` == 'Stable') %>% 
      select(`Class (Tau PET)`, Week = ADURW)) %>%
  select(`Class (Tau PET)`, Week, prediction = Ypred_class3, 
    lower = lower.Ypred_class3, upper = upper.Ypred_class3)

pred_slow <- 
  predictY(m.best.tau, data_pred_tau %>% filter(`Class (Tau PET)` == 'Slow decliner'), 
    var.time = "ADURW", draws = TRUE)$pred %>%
  as_tibble() %>%
  bind_cols(data_pred_tau %>% filter(`Class (Tau PET)` == 'Slow decliner') %>% 
      select(`Class (Tau PET)`, Week = ADURW)) %>%
  select(`Class (Tau PET)`, Week, prediction = Ypred_class1, 
    lower = lower.Ypred_class1, upper = upper.Ypred_class1)

pred_fast <- 
  predictY(m.best.tau, data_pred_tau %>% filter(`Class (Tau PET)` == 'Fast decliner'), 
    var.time = "ADURW", draws = TRUE)$pred %>%
  as_tibble() %>%
  bind_cols(data_pred_tau %>% filter(`Class (Tau PET)` == 'Fast decliner') %>% 
      select(`Class (Tau PET)`, Week = ADURW)) %>%
  select(`Class (Tau PET)`, Week, prediction = Ypred_class2, 
    lower = lower.Ypred_class2, upper = upper.Ypred_class2)
...
p2 <- ggplot(pred, aes(x=Years, y=prediction_raw, group = `Class (Tau PET)`)) +
  geom_line(aes(color = `Class (Tau PET)`)) +
  geom_ribbon(aes(ymin = lower_raw, ymax = upper_raw, fill = `Class (Tau PET)`), alpha = 0.2) +
  ylab('Predicted PACC (95% CI)') + 
  scale_x_continuous(breaks = 0:8) + 
  ylim(yrange) +
  guides(color = guide_legend(title = "PACC class", 
...
```{r make-suppfig-tree-base, include=FALSE}
png("suppfig-tree-base.png", width = 4.5*1.25, height = 5.75*1.2, units = "in", res = 300)
...
round(top_models[[1, 'std_err']], 3), ").",
  file = "_suppfig-tree-tau-pet-cap.txt")
```
...
{{< include _suppfig-tree-tau-pet-cap.txt >}}
...
```{r supptbl-clda-power}
#| results: asis
#| tbl-colwidths: [10,10,10,10,10,10,10]

em_effect %>%
  filter(Years %in% c(2, 4), Group != 'Stable Amyloid-') %>%
  mutate(
    N = case_when(
      Years == 2 ~ 500*(1-0.1),
      Years == 4 ~ 500*(1-0.2)),
